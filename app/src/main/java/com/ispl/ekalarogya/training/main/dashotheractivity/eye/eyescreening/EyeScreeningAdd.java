...
import com.ispl.ekalarogya.training.adapters.spinner.MemberArrayListAdapter;
...
public class EyeScreeningAdd extends AppCompatActivity implements RetroAPICallback, View.OnClickListener {
String searchData;
...
//    TextView tvEmpty;
    ...
    private static final String TAG = EyeScreeningAdd.class.getSimpleName();
    int page;
    String type,visionDefect="0",meeting_date,remark,village,member_id,head_id,status="0",gender,patient_id,member_name,es_id,age,guest_patient_name,patient_mobile;
...
//                    break;
                case "add":
                    isEdit=true;
                    tv_title.setText(getResources().getString(R.string.add_eye_screening));
                    village=i.getParcelableExtra("v_id");

                    if(i.getStringExtra("member_type")!=null){
                        isGuest =true;
                        ...
                        tvFamilyHeadName.setVisibility(View.VISIBLE);
                        spin_head_name.setVisibility(View.VISIBLE);
                        spinner_member_name.setVisibility(View.VISIBLE);
                        etGuestName.setVisibility(View.GONE);
                        etGuestMob.setVisibility(View.GONE);
...
////                    village=i.getStringExtra("v_id");
//                    tv_title.setText("View Eye Screening");
//
//                    break;
//                case "edit":
//                    isEdit=false;
...
//                }else {
                    ...
                    if (clickedItem.getId() != null) {
                        ...
                        member_name = clickedItem.getName();
                    } else {
                        member_id = clickedItem.getHead_id();
                        member_name = clickedItem.getHead_name();
                    }
                    gender=clickedItem.getGender();
                    age=clickedItem.getAge();
                    tv_age.setText(age);
                    gender=clickedItem.getGender();
                    updateGenderUI();
//                    gender=clickedItem.getGender();
...
//        }
    ...
    private void setupDATA() {
        getVillageList();
        if(!isGuest){
            Log.d(TAG, "setupDATA: i m not guest");
...
//                   headName.clear();
//                                            headLoad();
                                            getHeadList(villName.get(i).getVillage_id());
                                        }
                                    }
                                }
                            }
...
//                                List<VillageListModel> villageListModelList = new ArrayList<>();
                                ...
                                if (m_data!=null){
                                    for (int i=0;i<headName.size();i++){
                                        if (headName.get(i).getId().equalsIgnoreCase(m_data.getHead_id())){
                                            spin_head_name.setSelection(i);
                                            Log.d(TAG, "eyeScreenDataInit: village done"+head_id);
//                   headName.clear();
//                                            headLoad();
                                            getMemberList(headName.get(i).getId());
                                        }else if(headName.get(i).getId().equalsIgnoreCase(m_data.getHead_id())){
                                            spin_head_name.setSelection(i);
                                            Log.d(TAG, "eyeScreenDataInit: village done"+head_id);
//                   headName.clear();
//                                            headLoad();
...
//            tv_date.setError("Please select date");
        ...
        if (village==null){
            ...
            return false;}
//        if (patient_id==null){
//            ToastUtils.shortToast("Select Patient");
//            return false;}
        if (visionDefect==null){
            ToastUtils.shortToast(getResources().getString(R.string.select_vision_defect));
            return false;}

        if (head_id!=null&&member_id!=null&&head_id.equalsIgnoreCase(member_id)) {
            member_id=null;
        }
    ...
    private void submitdata() {
        UserService.addEyeScreening(this,status,village,patient_id,null,head_id,etGuestName.getText().toString(),member_name,etGuestMob.getText().toString(),gender,tv_date.getText().toString(),visionDefect,tv_age.getText().toString(),es_id,remark,this,ADD_DATA_REQUEST);
    }
    ...
    private void getDateDialog() {
        ...
        DatePickerDialog datePicker = new DatePickerDialog(EyeScreeningAdd.this, R.style.DatePickerTheme, new DatePickerDialog.OnDateSetListener() {
            @Override
